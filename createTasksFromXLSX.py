...
def postIssues( issue ):
    ...
    r = requests.post("http://redmine/projects/time-dev-1/issues.json",
...
def getIssue( issueId ):
    r = requests.get("http://redmine/projects/time-dev-1/issues/" + str(issueId) + ".json",
                    auth=(username, password)
                    )
...
def main():
    ...
    if args.user:
        ...
        username = input("Usuário do redmine: ")

    password = getpass.getpass("Senha: ")

    parentId = args.parent if args.parent else int(input("Id da tarefa pai: "))
